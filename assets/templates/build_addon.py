...
def build_manifest() -> str:
    return MANIFEST_TEMPLATE.read_text(encoding="utf-8").format(**ADDON_INFO)
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="nvda-addon-build-") as temp_dir:
        staging_dir = Path(temp_dir) / ADDON_INFO["name"]
        staging_dir.mkdir(parents=True, exist_ok=True)
        for source_path in sorted(ADDON_DIR.iterdir()):
            destination_path = staging_dir / source_path.name
            if source_path.is_dir():
                shutil.copytree(source_path, destination_path, ignore=_ignore_copy_entries)
            else:
